...
if (!LoadProdukDetail(trim($_GET['id']))) {
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (empty(trim($_POST['potong']))) {
			$potong = "Potong ditempat tidak boleh kosong";
		} else {
			$potong = FilterInput($_POST['potong']);
			$potong = EscapeString($potong);
		}
		if (empty(trim($_POST['hargapotongkr']))) {
			$hargapotongkr = "Harga potong ditempat tidak boleh kosong";
		} else {
			$hargapotongkr = FilterInput($_POST['hargapotongkr']);
			$hargapotongkr = EscapeString($hargapotongkr);
		}
		if ($potong == 'Tidak'){
			$hargapotongkr = 0;
		}
		if (empty($jumlah_beli_err) && empty($alamat_pembeli_err)) {
			//Insert Ke kEranjang
			if (InsertKeranjang($id_pembeli, $nama_lengkap, $alamat_pembeli, $jumlah_beli, $id, $nama_produk, $jenis_produk, $harga_produk, $satuan_produk, $foto_produk, $tanggal_pembelian, $potong,$hargapotongkr)) {
		...
		<form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
			...
			<div class="table-responsive">
				<table width="100%"" class=" table table-sthiped">
					...
					<tr>
						<td>Harga potong ditempat</td>
						<td>Rp. <input type="number" name="hargapotongkr" readonly value=<?php echo $hargapotong ?>> /Ekor</td>
					</tr>
					<tr>
						<td>Potong ditempat</td>
						<td><select name="potong" id="">
