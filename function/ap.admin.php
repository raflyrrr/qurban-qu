...
function InsertProduk($nama, $nama_produk, $deskripsi_produk, $jenis_produk, $harga_produk, $satuan_produk, $stok_produk,$hargapotong, $foto_produk, $tanggal_buat)
{
	global $FileItem, $FileDestination;
	$sql = "INSERT INTO ap_produk (nama_produk_pemilik, nama_produk, deskripsi_produk, jenis_produk, harga_produk, satuan_produk, stok_produk,hargapotong, foto_produk, tanggal_buat) VALUES(?,?,?,?,?,?,?,?,?,?)";
	if ($stmt = prepare($sql)) {
		$stmt->bind_param(
			"sssssssssi",
...
{
	global $id, $id_pembeli, $nama_pembeli, $alamat_pembeli, $jumlah_beli, $id_produk, $nama_produk, $jenis_produk, $harga_produk, $satuan_produk, $foto_produk, $status_pengiriman, $status_pembayaran, $tanggal_pembelian, $tanggal,$potong,$hargapotongkr;
	$sql = "SELECT id, id_pembeli, nama_pembeli, alamat_pembeli, jumlah_beli, id_produk, nama_produk, jenis_produk, harga_produk, satuan_produk, foto_produk, status_pengiriman, status_pembayaran, tanggal_pembelian, tanggal,potong,hargapotongkr FROM ap_keranjang WHERE id=?";
	if ($stmt = prepare($sql)) {
		$stmt->bind_param("i", $param_id);
		$param_id = $var_id;
		if ($stmt->execute()) {
			$stmt->store_result();
			$stmt->bind_result($id, $id_pembeli, $nama_pembeli, $alamat_pembeli, $jumlah_beli, $id_produk, $nama_produk, $jenis_produk, $harga_produk, $satuan_produk, $foto_produk, $status_pengiriman, $status_pembayaran, $tanggal_pembelian, $tanggal,$potong,$hargapotongkr);
			$stmt->fetch();
			if ($stmt->num_rows == 1) {
				return true;
			} else {
...
{
	global $id, $nama_produk_pemilik, $nama_produk, $deskripsi_produk, $jenis_produk, $harga_produk,$hargapotong , $satuan_produk, $stok_produk, $foto_produk, $tanggal_buat, $tanggal_update;
	$sql = "SELECT id, nama_produk_pemilik, nama_produk, deskripsi_produk, jenis_produk, harga_produk,hargapotong, satuan_produk, stok_produk, foto_produk, tanggal_buat, tanggal_update FROM ap_produk WHERE id=?";
	if ($stmt = prepare($sql)) {
		$stmt->bind_param("i", $param_id);
		$param_id = $var_id;
		if ($stmt->execute()) {
			$stmt->store_result();
			$stmt->bind_result($id, $nama_produk_pemilik, $nama_produk, $deskripsi_produk, $jenis_produk, $harga_produk,$hargapotong, $satuan_produk, $stok_produk, $foto_produk, $tanggal_buat, $tanggal_update);
			$stmt->fetch();
			if ($stmt->num_rows == 1) {
				return true;
			} else {
...
function LoadProdukContent($id, $nama_produk_pemilik, $nama_produk, $deskripsi_produk, $jenis_produk, $harga_produk, $satuan_produk, $stok_produk,$hargapotong, $foto_produk, $tanggal_buat, $tanggal_update)
{
	echo '
	<div class="row">
	<div id="content-wrapper">
      <div class="container-fluid">
		...
		<li class="nav-item">
		  <a class="nav-link active" data-toggle="tab" href="#home">Deskripsi Hewan</a>
		</li>		    
	  </ul>
		...
		<div class="table-responsive">
			<table width="100%"" class="table table-sthiped table-bordered table-hover">
			 <tr>
			  ...
			  <tr>
				<td>Berat</td>
				<td>' . $satuan_produk .' Kg</td>
			  </tr>
			  <tr>
				<td>Stok</td>
				<td>' . $stok_produk .' Ekor</td>
			  </tr>
			</table>
			</div>
		</div>
...
function UpdateProduk($nama, $nama_produk, $deskripsi_produk, $jenis_produk, $harga_produk, $satuan_produk, $stok_produk,$hargapotong, $foto_produk, $id)
{
	global $FileItem, $FileDestination;
	$sql = "UPDATE ap_produk SET nama_produk_pemilik = ?, nama_produk=?, deskripsi_produk=?, jenis_produk=?, harga_produk=?, satuan_produk=?, stok_produk=?,hargapotong=?, foto_produk=? WHERE id=?";
	if ($stmt = prepare($sql)) {
		$stmt->bind_param(
			"sssssssssi",
			$param_nama_produk_pemilik,
...
function LoadJenisProduk()
{
	...
	<h4>Jenis Hewan</h4>
	<br/>
	<a title="Tambah Jenis Produk" href="produk.jenis.php" class="btn btn-sm btn-primary"><i class="fas fa-shopping-cart"></i> Tambah Jenis Hewan</a>
	<br/>
	<br/>
	<div class="table-responsive">
    <table width="100%"" class="table table-sthiped table-bordered table-hover">
	...
	if ($TampilJenisProduk->num_rows > 0) {
      ...
      <td>      
      <a title="Edit Jenis Hewan" href="editKategori.php?id=' . $row['id'] . '"><i class="fas fa-fw fa-edit"></i></a>
      <a title="Delete Jenis Hewan" href="deleteKategori.php?id=' . $row['id'] . '" onClick=\'alert("Kategori berhasil dihapus");\' ><i class="fas fa-fw fa-trash"></i></a>
    ...
    <tr>
    <td colspan="7" align="center">
    Belum ada Jenis Hewan yang ditambahkan</td>
    </tr>
